...
Tritium decodes the SeaBird v3.1 \texttt{FD id} performance-marker prefix and
may ignore every marker. Markers never alter WCET-visible architectural
semantics, exception behavior, memory ordering, or lockstep comparison state;
the reference Tritium profile reports no marker optimizations in query leaf
\texttt{0x0006}.
...
Meisei Tritium Embedded Processor Preliminary Datasheet v1 Draft; SeaBird architecture 3.2 machine-readable specification and architecture manuals in this repository. This document defines an implementation profile and does not override parent instruction semantics.
