...
\fancyhead[L]{\small\sffamily SeaBird ISA Volume 3}\fancyhead[R]{\small\sffamily Version 3.0 RC1}
...
\begin{enumerate}
    ...
    \item In active pointer-width slots, hardware pushes old CPL, old SP, FLAGS, return IP, vector, and error code. Old CPL/SP are zero when no privilege change occurs.
...
CR3 contains the 4 KiB-aligned physical root address. In Tetra mode only root entries 0--3 are reachable. At each non-leaf level, PFN supplies the next table address. A leaf at the lowest level maps 4 KiB; PS at the next level maps 2 MiB; PS one level higher maps 1 GiB. PS in any other position is reserved and causes PAGE\_FAULT.
...
\begin{itemize}
    ...
    \item \textbf{Optional extensions:} FP, SIMD, AVX, CRYPTO, DSP, PIO, TXN, VIRT, PMU, SHSTK, IBT, BCD, and MLAUNCH are independently reported.
\end{itemize}
\end{document}
