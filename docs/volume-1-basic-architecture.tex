...
\fancyhead[L]{\small\sffamily SeaBird ISA Volume 1}\fancyhead[R]{\small\sffamily Version 3.0 RC1}
...
\textbf{Optional register windowing:} Implementations may provide a register-window or register-renaming mechanism beneath the architectural ABI. Windowing must be invisible to normal software: architectural R0--R31 names, caller/callee preservation rules, exception save/restore, and binary translation behavior remain exactly as specified. A windowed implementation must flush or materialize architectural register state before traps, interrupts, context switches, and debug visibility events.
...
Scalar floating-point mnemonics use an explicit \texttt{.s} suffix for the
IEEE binary32 encoding and \texttt{.q} for IEEE binary128, for example
\texttt{FADD.S}, \texttt{FLD.Q}, and \texttt{FCVTI.Q}. An unsuffixed scalar
floating-point mnemonic denotes the binary64 form. The suffix selects the
corresponding operand-width or scalar lane-width prefix and is part of the
canonical assembly spelling, not a separate architectural instruction.
...
Reserved base opcodes & 0xD8--0xFD & 38 reserved values & 1 \\
...
\begin{itemize}
    \item \textbf{AVX (ExtGroupID = 0x01):} sub-opcodes `0x00`..`0x2A` (43 entries) mapped to the AVX instruction list in the order shown in the Advanced Vector table. Reserved AVX sub-opcodes 0x2B--0x2F are held for future widening so the group remains naturally aligned.
    ...
    \item \textbf{SYSX (ExtGroupID = 0x04):} assigned sub-opcodes in `0x00`..`0x1A` are mapped by the architectural system-extension table; unlisted values are reserved.
    \item \textbf{FPX (ExtGroupID = 0x05):} sub-opcodes `0x00`..`0x0F` encode the sixteen extended scalar floating-point operations in table order.
\end{itemize}
